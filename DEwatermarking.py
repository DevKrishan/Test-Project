...
class watermarking():
    def __init__(self, watermark_path="watermark.jpg", ratio=0.1, wavelet="haar",level=2, x = [0.1], cover_image = "lena.jpg"):
        self.level = level
        self.wavelet = wavelet
        self.ratio = x[0]
        self.shape_watermark = cv2.imread(watermark_path, 0).shape
        assert not isinstance(watermark_path,type(None)), 'image not found'
        assert not isinstance(cover_image,type(None)), 'image not found'
